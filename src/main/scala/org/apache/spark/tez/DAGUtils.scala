 ...
 * 
 */
class DAGUtils(sc:SparkContext, func:Function2[_,_,_]) {
  ...
  val tezClient = new TezClient(sc.appName, new TezConfiguration(sc.hadoopConfiguration));
  this.tezClient.addAppMasterLocalResources(this.localResources);
  tezClient.start();
 
  private val dagBuilder = new DAGBuilder(this.tezClient, this.localResources, this.tezConfiguration, sc.appName + "_out")
...
//    rdd.getNarrowAncestors
  ...
  private def normalizeForTez(rdd: RDD[_]): Unit = {
    ...
    def visit(rdd: RDD[_]) {
      ...
      if (rdd.dependencies(0).rdd.isInstanceOf[HadoopRDD[_, _]] || rdd.dependencies(0).rdd.isInstanceOf[ParallelCollectionRDD[_]]) {
  ...
  private def serializePartitions(partitions:Array[Partition], serializer:SerializerInstance){
    if (partitions.size > 0 && partitions(0).isInstanceOf[ParallelCollectionPartition[_]]){
      ...
      for (partition <- partitions) {
        val partitionPath = new Path(sc.appName + "_p_" + partitionCounter)
